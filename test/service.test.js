...
describe('Service Integration Tests with Multiple Stubs', () => {
    ...
    // New tests for deleteItemById
    it('should delete an item from the primary repository', () => {
        primaryRepositoryStub.deleteItemById.withArgs(1).returns(true); // Set up the stub to return true

        const result = service.deleteItemById(1);
        expect(result).to.be.true; // Expect true since the item was deleted
    ...
    it('should return false if trying to delete a non-existent item', () => {
        primaryRepositoryStub.deleteItemById.withArgs(3).returns(false); // Set up the stub to return false

        const result = service.deleteItemById(3);
        expect(result).to.be.false; // Expect false since the item was not found
